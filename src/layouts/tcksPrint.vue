        ...
        <div class="col">
          <div class="row">
            ...
            <q-select class="col" v-model="stores.val" :options="stores.opts" label="Sucursales" filled
              option-label="name" @update:model-value="index" dense />
          </div>
          ...
          <q-table class="my-sticky-header-table" title="Fromas de Pago" :rows="fpas.body" row-key="TICKET"
            ...
            <template v-slot:top-right>
              <div class="row">
                ...
                <q-separator spaced inset vertical dark />
                <q-btn class="col" color="primary" icon="archive" dense no-caps flat round @click="exportTable" />
...
if (VDB.session.rol == 'aux' || VDB.session.rol == 'gen' || VDB.session.rol == 'aud' || VDB.session.rol == 'root' || VDB.session.rol == 'adm') {
  init()
} else {
