<template>
  <q-page padding>
    <q-table title="Depositos" :rows="bascket" row-key="id" :rows-per-page-options="[0]" separator="cell"
      :columns="table.columns" :filter="table.filter">
      <template v-slot:top="props">
        <div class="col-2 q-table__title q-mr-xl">Depositos</div>

        <q-space />
        <q-select v-model="stores.val" :options="stores.opts" label="Sucursal" option-label="name" filled
          style="width: 200px;">
        ...
        <q-select v-model="status.val" :options="status.opts" label="Estado" option-label="name" filled
          style="width: 200px;">
        ...
        <q-separator spaced inset vertical />
        <q-btn color="primary" icon="download" @click="exportTable" flat round />
        <q-separator spaced inset vertical />
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td key="status" :props="props">
            <q-select :bg-color="colors[props.row.status.id - 1]" v-model="props.row.status" :options="status.opts"
              label="Estado" filled option-label="name" dense @update:model-value="changeStatus(props.row)" />
          </q-td>
...
soc.on('ChangeTicket', (param) => {
  ...
  if (inx >= 0) {
    ...
    $q.notify({ message: `El deposito de $ ${param.import} se agrego el ticket ${param.ticket}`, type: 'positive', position: 'center' })
  }
})
...
const exportTable = async () => {
  ...
  bascket.value.forEach(async (row) => {
    worksheet.addRow([
      row.id,
      row.send_by,
      row.store.name,
      ...
      row.status.name,
      row.ticket,
      `http://192.168.10.160:1920/Assist/public/storage/${row.picture}`,
    ])
  });
