<template>
  ...
  <q-dialog v-model="warehouseSel.state">
    <q-card>
      <q-form @submit="() => { warehouseSel.val ?  nextState() : $q.notify({message:'necesito la llave :x', type:'negative',position:'center'})}">
        <q-card-section class="row items-center">
          <q-avatar icon="warning" color="orange-14" text-color="white" />
          <span class="q-mx-sm text-h6">Iniciar CheckOut #{{ partition }}</span>
        </q-card-section>
        <q-card-section>
          <q-input v-model="warehouseSel.val" filled type="text" label="LLave" auto-focus />
        </q-card-section>
        <q-card-actions vertical align="center">
          <q-btn flat label="Si" color="primary" type="submit" :disable="warehouseSel.val == null" />
        </q-card-actions>
      </q-form>
    </q-card>
  </q-dialog>
...
const ordersdb = computed(() => $restockStore.partitions.filter(o => o._status == 8 && o.requisition._workpoint_from == VDB.session.store.id_viz))
const init = async () => {
...
const nextState = async () => {
  ...
  if (savesupply.status == 200) {
    ...
    partition.value = null
    $router.push(`/distribute/checkin/${savesupply.data.partition.id}/?key=${savesupply.data.partition.entry_key}`)
    $q.loading.hide();
  } else {
...
init();
// document.title = "Vhelpers/Checkin";
</script>
