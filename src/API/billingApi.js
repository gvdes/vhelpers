...
export default {
  ...
  crearFacturaInterna(data) {
    return assist.post(`/billing/crearFacturaInterna`, data).then(done => done.data).catch(fail => { return { fail } });
  },
  nextState(data) {
  ...
  readRFC(data) {
    return assist.post(`/billing/readRFC`, data).then(done => done.data).catch(fail => { return { fail } });
  },
}
