          ...
          <q-expansion-item v-for="parent in treePermissions" :key="parent.id" expand-separator>
            <template #header>
              <div class="column full-width">
                ...
                <div class="row q-gutter-sm q-mt-xs">
                  <q-checkbox v-model="parent.permission" label="Acceso" @update:model-value="onParentChange(parent)" />
                </div>
              </div>
            </template>
            <q-separator />
            <q-item v-for="child in parent.children" :key="child.id" class="q-ml-lg">
              <div class="column full-width">
                ...
                <div class="row q-gutter-sm q-mt-xs">
                  <q-checkbox v-model="child.permission" label="Acceso"  @update:model-value="onChildChange(parent)" />
                </div>
                <q-separator />
              </div>
            </q-item>
...
const onParentChange = (parent) => {
  parent.children.forEach(c => {
    c.permission = parent.permission
  })
}

const onChildChange = (parent) => {
  const anyActive = parent.children.some(c => c.permission)
  parent.permission = anyActive
}
const assigned = new Map(
  props.rol?.modules?.map(m => [m.id, true]) ?? []
)
...
const collectPermissions = () => {
  ...
  treePermissions.value.forEach(p => {
    if (p.permission) out.push(p.id)

    p.children.forEach(c => {
      if (c.permission) out.push(c.id)
    })
  })
...
onMounted(() => {
  ...
  treePermissions.value = props.permissions.map(p => ({
    ...p,
    permission: assigned.get(p.id) ?? false,
    children: (p.children || []).map(c => ({
      ...c,
      permission: assigned.get(c.id) ?? false
    }))
  }))
})
</script>
