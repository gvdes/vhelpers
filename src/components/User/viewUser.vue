      ...
      <q-tab-panels v-model="tab" animated>
        <q-tab-panel name="personal">
          <q-card-section class="q-gutter-sm">
            <q-input v-model="editableUser.complete_name" label="Nombre(s)" filled dense :rules="rules.name"
              :class="changedClass('complete_name')" :disable="canView" />
      ...
      <q-card class="media-card">
        <q-bar>
          <div class="text-bold ellipsis">
            {{ editableUser?.nick }} — {{ editableUser?.complete_name }}
          </div>
          <q-space />
    ...
    <q-dialog v-model="checador" persistent>
      <q-card style="min-width: 350px">
        ...
        <q-card-section class="q-pt-none">
          <q-input dense v-model="editableUser.id_rc" autofocus @keyup.enter="prompt = false" :error="idrcvalid"
...
const rules = {
  complete_name: [v => !changed('complete_name') || !!v || 'Requerido'],
...
const idrcvalid = computed(() => {
  ...
  let nkmil = arr.filter((e) => e.id_rc == editableUser.value.id_rc);
...
const hasChanges = computed(() => {
  ...
  return (
    e.complete_name !== o.complete_name ||
    e.nick !== o.nick ||
    e.email !== o.email ||
...
const changedPayload = computed(() => {
  ...
  if (e.complete_name !== o.complete_name) p.complete_name = e.complete_name
  ...
  if (e.id_rc !== o.id_rc) p.id_rc = e.id_rc
