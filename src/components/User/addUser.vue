  ...
  <q-form ref="formRef" @submit.prevent="addForm">
    ...
    <q-tab-panels v-model="tab" animated keep-alive transition-prev="scale" transition-next="scale">
      <q-tab-panel name="personal">
        ...
        <q-card-section>
          <q-input v-model="form.complete_name" type="text" label="Nombre Completo" filled dense clearable :rules="[validFullName, required]"
            debounce="300" />
...
const validFullName = () => {
  if (!form.value.complete_name) return true
  const full = form.value.complete_name.toLowerCase()
  const exists = props.users.some(u =>
    u.complete_name.toLowerCase() === full
  )
...
const addForm = async () => {
  ...
  data.append('complete_name', form.value.complete_name)
