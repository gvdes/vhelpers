...
const block = computed(() => { return st => props.checkState ? props.blockStates.some(e => e == st.status) && ![1,2,5,6,12,18,19,21,22,24].includes($user.session.credentials._rol) : false; })
      ...
      let options = resp.map(p => {
        ...
        if (props.checkState) {
          if (props.wkpToVal) {
            ...
            p.stateToVal = stock?.pivot?.state_data ?? p.status
            console.log(stock)
          } else { p.stateToVal = p.status; }
        } else { p.stateToVal = p.status; }
        return p;
      });
      update(
  ...
  if (data.value.target.length) {
    ...
    if (resp.fail) {
      ...
      if (resp.id) {
        if (props.checkState) {
          if (props.wkpToVal) {
            const stock = resp.stocks?.find(s => s.id == props.wkpToVal)
            resp.stateToVal = stock?.pivot?.state_data ?? resp.status
            console.log(stock)
          } else { resp.stateToVal = resp.status; }
        } else { resp.stateToVal = resp.status; }
        selItem(resp);
        putFocus();
      } else {
