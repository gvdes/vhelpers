<template>
  <q-card>
    ...
    <q-card-actions align="right">
      ...
      <q-btn flat label="Terminar" color="positive" @click="boxClose"  />
    </q-card-actions>
  </q-card>
</template>
...
const boxClose = async () => {
  $q.loading.show({message:'Arqueo de Caja'})
  let data = {
    close:closeBox.value,
    cash:props.cash,
    total:total.value,
    // uid: VDB.session.credentials.id,
    created_by:VDB.session.credentials
  }
  console.log(data)
  const resp = await cashApi.countCash(data)
  console.log(resp)
  if(resp.fail){
    console.log(resp);
  }else{
    console.log(resp);
    $q.notify({message:'Arqueo Realizado',type:'positive',position:'top'})
